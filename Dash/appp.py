...
from pycaret.time_series import load_model, predict_model  # เพิ่มการนำเข้า pycaret
...
# โหลดโมเดลที่ฝึกแล้ว
model_pm25 = load_model('pm25_model')  # เปลี่ยนชื่อไฟล์โมเดลให้ถูกต้อง
model_humidity = load_model('humidity_model')  # เปลี่ยนชื่อไฟล์โมเดลให้ถูกต้อง
model_temperature = load_model('temperature_model')  # เปลี่ยนชื่อไฟล์โมเดลให้ถูกต้อง

# ทำการพยากรณ์
pm25_predictions = predict_model(model_pm25, data=pm25_df)
humidity_predictions = predict_model(model_humidity, data=humidity_df)
temperature_predictions = predict_model(model_temperature, data=temperature_df)

# คำนวณค่าเฉลี่ยรายวันในช่วง 7 วันที่ผ่านมา
pm25_avg = pm25_predictions['Label'].mean()  # ใช้ผลพยากรณ์
humidity_avg = humidity_predictions['Label'].mean()  # ใช้ผลพยากรณ์
temperature_avg = temperature_predictions['Label'].mean()  # ใช้ผลพยากรณ์
...
# สร้างกราฟ PM2.5
fig_pm25 = px.line(pm25_df, x='datetime', y=pm25_predictions['Label'], title='PM2.5 Prediction')
...
# สร้างกราฟความชื้น
fig_humidity = px.line(humidity_df, x='datetime', y=humidity_predictions['Label'], title='Humidity Prediction')
...
# สร้างกราฟอุณหภูมิ
fig_temperature = px.line(temperature_df, x='datetime', y=temperature_predictions['Label'], title='Temperature Prediction')
